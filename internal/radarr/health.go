...
// Radarr keeps its own health checks — indexers that stopped answering, a
// download client that is unreachable, a root folder that vanished. They are
// the reason a queue can be empty and a library can be missing half its films
// while nothing looks broken from the outside.
...
// Radarr reports free space per root folder but not the size of the disk
// behind it, so there is no percentage to give here — system_disk_usage is the
// tool that knows that.
type RootFolderSpace struct {
	Path            string `json:"path"`
	Accessible      bool   `json:"accessible"`
	FreeBytes       uint64 `json:"free_bytes,omitempty"`
	UnmappedFolders int    `json:"unmapped_folders,omitempty" jsonschema:"folders inside this root that Radarr has no movie for"`
}
...
type Health struct {
	...
	Issues []HealthIssue `json:"issues,omitempty" jsonschema:"Radarr's own health checks, only the ones currently failing"`

	QueueCount        int  `json:"queue_count"`
	QueueUnknownCount int  `json:"queue_unknown_count,omitempty" jsonschema:"queue items the download client holds that Radarr cannot match to a movie"`
...
// GetHealth reports whether Radarr itself is in a state to do its job.
func GetHealth(ctx context.Context) (Health, error) {
	...
	if statusErr == nil {
		...
		h.Warnings = append(h.Warnings, "could not read Radarr's version: "+statusErr.Error())
	}
	...
	if issuesErr == nil {
		...
		h.Warnings = append(h.Warnings, "could not read Radarr's health checks: "+issuesErr.Error())
	}
...
func healthWarnings(h Health) []string {
	...
	if h.QueueErrors {
		out = append(out, "the download queue has items in error — radarr_queue_status shows which")
	}
	if h.QueueUnknownCount > 0 {
		out = append(out, fmt.Sprintf(
			"%d queue %s held by the download client that Radarr cannot match to a movie",
